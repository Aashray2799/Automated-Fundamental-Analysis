...
if ticker in df['Ticker'].values:
    ...
    analysis_scope = st.radio("Compare within:", ["Sector", "Industry"])
...
grading_metric = st.selectbox("Metric for grading breakdown", available_metrics, key="grading")
...
if stock_val is not None and not values.empty:
    ...
    st.warning("⚠️ No data available for grading.")

# --- README-style Explanation ---
st.markdown("---")
st.header("📖 How the Grading System Works")

st.markdown("""
### Automated Fundamental Analysis via Python

This program rates stocks out of 100 based on valuation, profitability, growth, and price performance metrics, **relative to sector**.

📁 View: `StockRatings-04.05.22.csv`
...
- Use the 90th percentile (or 10th for inverse metrics like P/E)
- Divide the std dev by 3 to compute **“Change” steps**

Grades are assigned like this:
...
These grades are then converted to numbers using a scoring system:
